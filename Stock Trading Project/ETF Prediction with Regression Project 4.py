...
actual_trend = np.asarray(y_test)[len(y_test) - 1] - np.asarray(y_test)[len(y_test) - 2]
...
prediction_trend = np.asarray(predicted_price_lin)[len(predicted_price_lin) - 1] - np.asarray(predicted_price_lin)[len(predicted_price_lin) - 2]
...
if (prediction_trend > 0 and actual_trend >= 0) or (prediction_trend >= 0 and actual_trend > 0) and last_state == 'sell':
...
elif (prediction_trend < 0 or actual_trend < 0) and last_state == 'buy':
